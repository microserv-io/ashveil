...
describe('motion review body selection', () => {
  it('offers the fitted body first and keeps the one it replaces beside it', () => {
    expect(REVIEW_BODIES.map((body) => body.id)).toEqual(['masculine-v3', 'masculine-v2', 'masculine-v1'])
    ...
    expect(MASCULINE_V3_BODY.profile).not.toBe(MASCULINE_V1_BODY.profile)
  })
