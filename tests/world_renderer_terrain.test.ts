...
describe('compiled-zone terrain presentation', () => {
  ...
  it('builds animated transparent water from compiled river queries and disposes it', () => {
    ...
    const waterDepth = water.mesh.geometry.getAttribute('waterDepth')
    ...
    expect(Math.min(...Array.from({ length: waterDepth.count }, (_, index) => waterDepth.getX(index)))).toBe(0)
    expect(Math.max(...Array.from({ length: waterDepth.count }, (_, index) => waterDepth.getX(index)))).toBeGreaterThan(6)
    ...
    expect(water.mesh.material.uniforms.shallowColor!.value.getHex()).toBe(0x42bde8)
    expect(water.mesh.material.uniforms.deepColor!.value.getHex()).toBe(0x075ca8)
    expect(water.mesh.material.uniforms.foamColor!.value.getHex()).toBe(0xf4f0dc)
    expect(water.mesh.material.vertexShader).toContain('maximumDisplacement = min(0.075, waterDepth * 0.45)')
    expect(water.mesh.material.vertexShader).toContain('clamp(rawDisplacement, -maximumDisplacement, maximumDisplacement)')
    expect(water.mesh.material.fragmentShader).toMatch(/litFoamColor = foamColor \* ambientFactor/)
    expect(water.mesh.material.fragmentShader).toContain('mix(color, litFoamColor, foam * 0.72)')
    expect(water.mesh.material.fragmentShader).toContain('float waterNoise(vec2 point)')
    expect(water.mesh.material.fragmentShader).toContain('float channelStroke = ribbon * dash * distanceFade')
    expect(water.mesh.material.fragmentShader).toContain('1.0 - smoothstep(0.02, 1.25, vWaterDepth)')
